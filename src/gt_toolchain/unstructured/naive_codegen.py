...
"""
    ...
    def visit_ForK(self, node, **kwargs) -> str:
        ...
        return self.render_node(
            node,
            dict(horizontal_loops=horizontal_loops,
                 k_loop=k_loop)
        )
    ...
    def visit_HorizontalLoop(self, node, **kwargs) -> str:
        ...
        return self.render_node(
            node,
            dict(stringified_location_type=stringified_location_type,
                 ast=ast)
        )
    ...
    def visit_ReduceOverNeighbourExpr(self, node, **kwargs) -> str:
        ...
        return self.render_node(
            node,
            dict(right_loc_type=right_loc_type,
                 loc_type=loc_type,
                 init=init,
                 operation=operation,
                 right=right,
    ...
    def visit_LiteralExpr(self, node, **kwargs) -> str:
        return self.render_node(
            node,
            dict(c_data_type=data_type_to_c(node.data_type),
    ...
    def visit_Computation(self, node, **kwargs) -> str:
        ...
        ctor_field_params = ", ".join('dawn::{loc_type}_field_t<LibTag, {data_type}>& {name}'.format(loc_type=location_type_to_str_singular_lower(
            p.location_type), name=p.name, data_type=data_type_to_c(p.data_type)) for p in node.params)
        ...
        return self.render_node(
            node,
            dict(api_field_declarations=api_field_declarations,
