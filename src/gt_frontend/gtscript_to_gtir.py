...
class CallCanonicalizer(eve.NodeModifier):
    def visit_Call(self, node: Call):
        ...
        if node.func in neighbor_selector_mapping:
            return Call(func="neighbors", args=[self.visit(node.args[0]), neighbor_selector_mapping[node.func]])
...
class GTScriptToGTIR(eve.NodeTranslator):
    ...
    def visit_LocationSpecification(self, node : LocationSpecification, **kwargs):
        ...
        return gtir.LocationComprehension(name=node.name.id,
                                          chain=gtir.NeighborChain(elements=[loc_type]),
                                          of=gtir.Domain())
    ...
    def visit_LocationComprehension(self, node : LocationComprehension, **kwargs):
        ...
        chain = gtir.NeighborChain(
            elements=[self._resolve_symbol(loc_type.id, expected_type=gtir.common.LocationType) for loc_type in
                      node.iter.args[1:]])

        return gtir.LocationComprehension(name=node.target.id,
                                          chain=chain,
                                          of=of)
    ...
    def visit_Call(self, node : Call, **kwargs):
        ...
        # reductions
        reduction_mapping = {"sum": gtir.ReduceOperator.ADD,
        ...
        if node.func in reduction_mapping:
            ...
            return gtir.NeighborReduce(op=op, operand=operand, neighbors=neighbors,
    ...
    def visit_Name(self, node : Name, location_stack):
        ...
        if issubclass(self.symbol_table[node.id], Field):
            return gtir.FieldAccess(name=node.id, location_type=location_stack[-1].chain.elements[-1],
                                    subscript=gtir.LocationRef(name=location_stack[0].name)) # todo: just visit the subscript symbol
    ...
    def visit_SubscriptSingle(self, node : SubscriptSingle, location_stack):
        ...
        if issubclass(self.symbol_table[node.value.id], Field):
            ...
            return gtir.FieldAccess(name=node.value.id, subscript=gtir.LocationRef(name=node.index),
                                    location_type=location_stack[-1].chain.elements[-1])

        raise ValueError()

    def visit_Assign(self, node : Assign, **kwargs):
        return gtir.AssignStmt(left=self.visit(node.target, **kwargs),
                               right=self.visit(node.value, **kwargs))
    ...
    def visit_Stencil(self, node : Stencil, **kwargs):
        ...
        for stmt in node.body:
            horizontal_loops.append(
                gtir.HorizontalLoop(stmt=self.visit(stmt, location_stack=location_stack, **kwargs), location=primary_location))

        return gtir.Stencil(
            vertical_loops=[gtir.VerticalLoop(loop_order=loop_order,
    ...
    def visit_Computation(self, node : Computation):
        ...
        for arg in node.arguments[1:]:
            ...
            primary_location_type, vtype = arg_type.args  # todo: sparse fields: location_types*, vtype

            assert isinstance(primary_location_type, common.LocationType)
            assert isinstance(vtype, common.DataType)

            field_args.append(gtir.UField(name=node.name, vtype=vtype,
