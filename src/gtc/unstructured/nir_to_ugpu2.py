...
class NirToUgpu(eve.NodeTranslator):
    ...
    def visit_HorizontalLoop(self, node: nir.HorizontalLoop, **kwargs):
        sids = []
        # print(common.LocationType(node.location_type).name)

        sids.append(ugpu2.SidComposite(name=str(common.LocationType(node.location_type).name)))
        debug(sids)
        #     field_accesses = eve.FindNodes().by_type(nir.FieldAccess, node.stmt)

        #     primary_sid_composite = ugpu2.SidComposite(
        #         chain=ugpu2.NeighborChain(chain=[node.location_type]), entries=[]
        #     )
        ...
        #             primary_sid_composite.entries.add(ugpu2.SidCompositeEntry(name=acc.name))
        #         else:
        #             assert (
        ...
        #                 other_sids[secondary_loc] = ugpu2.SidComposite(
        #                     chain=ugpu2.NeighborChain(chain=[node.location_type, secondary_loc]),
        #                     entries=[],
        #                 )
        #             other_sids[secondary_loc].entries.add(ugpu2.SidCompositeEntry(name=acc.name))
        ...
        kernel = ugpu2.Kernel(
            ast=[],
            name="bla",
            primary_connectivity="tmp",
            primary_sid="tmp2",
            connectivities=[],
            sids=[],
        )
        return kernel, ugpu2.KernelCall(name="bla")
