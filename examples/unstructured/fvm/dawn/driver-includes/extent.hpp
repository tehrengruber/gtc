...
// Access dimensions with `auto dim_extent = get<D>();`, where D = 0,1,2
// (=i,j,k) and minus/plus component with `get<C>(dim_extent)`, where C = 0
// (minus), 1 (plus) (Note, need `using std::get` for ADL to kick-in if std <
// c++20)
...
namespace detail {
template <int I> struct get_impl;

template <> struct get_impl<0> {
  using type = bool;
  constexpr type operator()(unstructured_extent const &extent) {
    return extent.horizontal;
  }
};
template <> struct get_impl<1> {
  using type = std::array<int, 2>;
  constexpr type operator()(unstructured_extent const &extent) {
    return extent.vertical;
  }
};
...
constexpr auto get(unstructured_extent const &extent)
    -> decltype(detail::get_impl<I>{}(extent)) {
