...
from gt_toolchain.unstructured import naive_codegen, sir_to_naive  # noqa: F401
...
class PVisitor(eve.PathNodeVisitor):
    def visit_Node(self, node, *, path_info, **kwargs):
        print("--->", [(p.node.__class__.__name__, p.kind.name, p.member) for p in path_info])
        self.generic_visit(node, path_info=path_info)


PVisitor().visit(stencil)
