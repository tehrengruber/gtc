...
namespace gridtools {
    namespace sid {
        namespace rename_all_dimensions_impl_ {
            template <class NewHymapKeys, class Map>
            auto remap(Map map) {
            ...
            template <class NewHymapKeys, class Sid>
            stride_kind_wrapper<NewHymapKeys, decltype(sid_get_strides_kind(std::declval<Sid const &>()))>
            sid_get_strides_kind(renamed_all_sid<NewHymapKeys, Sid> const &);

            template <class NewHymapKeys, class Sid>
            decltype(remap<NewHymapKeys>(sid_get_strides(std::declval<Sid const &>()))) sid_get_strides(
                renamed_all_sid<NewHymapKeys, Sid> const &obj) {
                return remap<NewHymapKeys>(sid_get_strides(obj.m_impl));
            }

            template <class NewHymapKeys, class Sid>
            decltype(remap<NewHymapKeys>(sid_get_lower_bounds(std::declval<Sid const &>()))) sid_get_lower_bounds(
                renamed_all_sid<NewHymapKeys, Sid> const &obj) {
                return remap<NewHymapKeys>(sid_get_lower_bounds(obj.m_impl));
            }

            template <class NewHymapKeys, class Sid>
            decltype(remap<NewHymapKeys>(sid_get_upper_bounds(std::declval<Sid const &>()))) sid_get_upper_bounds(
                renamed_all_sid<NewHymapKeys, Sid> const &obj) {
