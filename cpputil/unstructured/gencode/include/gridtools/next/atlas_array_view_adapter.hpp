...
auto sid_get_strides(ArrayView<Value, Rank> const &view) {
  return sid_adapter_impl_::to_hymap<std::make_index_sequence<Rank>>{}(
      view.strides());
}

template <typename Value, int Rank>
std::tuple<Value, std::integral_constant<
                      int, Rank>> /* TODO this is not a proper kind as we don't
                                     guarantee same strides */
sid_get_strides_kind(ArrayView<Value, Rank> const &);

template <typename Value, int Rank>
sid_adapter_impl_::lower_bounds_t<std::make_index_sequence<Rank>>
sid_get_lower_bounds(ArrayView<Value, Rank> const &) {
  return {};
}
